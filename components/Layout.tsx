...
const Layout: React.FC<LayoutProps> = ({ children, activeView, onViewChange, user, onLogout, lang, onToggleLang }) => {
  ...
  return (
    <div className={`flex h-screen bg-[#F8FAFC] text-slate-900 overflow-hidden font-sans ${isRtl ? 'flex-row-reverse' : 'flex-row'}`} dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <aside className="w-72 bg-white border-x border-slate-200 flex flex-col z-50 shadow-sm transition-all duration-300">
        <div className="p-8 flex flex-col items-center gap-6 border-b border-slate-100">
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-[#F8FAFC]">
        ...
        <header className="h-24 bg-white/80 backdrop-blur-xl border-b border-slate-200 flex items-center justify-between px-12 sticky top-0 z-40">
          <div className="flex items-center gap-8">
            <button 
              ...
              className="group flex items-center gap-3 px-5 py-2.5 bg-white border border-slate-200 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] hover:bg-slate-50 transition-all text-slate-700"
            >
              <Globe size={14} className="text-emerald-600 group-hover:rotate-12 transition-transform" />
              <span>{isRtl ? 'English Access' : 'النفاذ بالعربية'}</span>
            </button>
          ...
          <div className="flex items-center gap-6">
            ...
            <button className="p-3 bg-slate-50 border border-slate-100 rounded-2xl text-slate-400 hover:text-emerald-600 transition-all relative group">
              <Bell size={20} />
            ...
            <div className="relative" ref={dropdownRef}>
              <button 
                ...
                className={`flex items-center gap-4 p-2 rounded-2xl border transition-all duration-300 ${
                  isProfileOpen ? 'bg-emerald-50 border-emerald-200 ring-4 ring-emerald-500/5' : 'bg-white border-slate-200 hover:bg-slate-50'
                }`}
              >
                <div className="w-10 h-10 bg-emerald-600 text-white rounded-xl flex items-center justify-center font-black shadow-lg shadow-emerald-600/20">
              ...
              {isProfileOpen && user && (
                <div className={`absolute top-full mt-4 w-80 bg-white border border-slate-200 rounded-[2.5rem] shadow-2xl p-8 space-y-8 animate-in fade-in slide-in-from-top-4 duration-300 ${isRtl ? 'left-0' : 'right-0'}`}>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
           ...
           {/* Enterprise Footer */}
           <footer className="mt-32 bg-white border-t border-slate-200 p-16 lg:p-24 relative overflow-hidden">
              <div className="absolute top-0 right-0 w-96 h-96 bg-emerald-500/5 blur-[120px] rounded-full pointer-events-none"></div>
              
              <div className={`max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-20 relative z-10 ${isRtl ? 'text-right' : 'text-left'}`}>
